...
def save_cvs(filename,args):
    with open(filename, "w") as f:
        ...
        fieldnames = ['time','year','month','day','hour','min','sec','number of passage', 'pos_ii[0]', 'pos_ii[1]',
                      ...
                      'wind[2]', 'cL', 'cD', 'aoa','S', 'mass', 'heat_rate', 'heat_load', 'T_r', 'q', 'gravity_ii[0]', 'gravity_ii[1]',
        ...
        for index in range(0 , len(s.orientation.time)):
            ...
            [s.orientation.time[index] ,s.orientation.year[index], s.orientation.month[index], s.orientation.day[index],
             ...
             s.physical_properties.S[index], s.performance.mass[index] ,s.performance.heat_rate[index],s.performance.heat_load[index],s.performance.T_r[index],s.performance.q[index],
