...
def RK4(f,h,t,y,m,T_ijk, index_phase_aerobraking, simulation):
    ...
    y_n = y + 1/6*(k_1 + 2*k_2 + 2*k_3 + k_4)
    t_n = t + h

    # Check terminal events
    breaker, impact_breaker = events(t_n, y_n, t, y, m, T_ijk, index_phase_aerobraking, simulation)
    return y_n,t_n,breaker,impact_breaker


def events(t_n, y_n, t_0, y_0, m, T_ijk, index_phase_aerobraking,simulation):
    ...
    while True:
        ...
        if index_phase_aerobraking == 0:   # 1 step simulation
            try:  # if the dragpassage key has been defined and is set to true, stop the simulation when altitude = 160 km
