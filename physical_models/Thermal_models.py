...
# Blunt Body
def heatrate_convective(rn,p,v,rho):
    k = 1.7623*10**(-4)  #kg^0.5/m
    index = 0
    q_conv = (k*math.sqrt(rho/rn)*v**3)*10**-4 #W/cm^2
    return q_conv

def heatrate_radiative(rn,p, v,rho):
    C = 4.736*10**4
    b = 1.22
    ...
    fun = interp1d(vf,fv)
    a = 1.072*10**6*v**(-1.88)*rho**(-0.325)
    f = fun(v)
    q_rad = (C*(rn**a) * (rho**b) * f) #W/cm^2
    return q_rad

# Flat Plates (from Shaaf and Chambre)
def heatrate_convective_maxwellian(S,T,a,p):
    alpha = a.aoa
